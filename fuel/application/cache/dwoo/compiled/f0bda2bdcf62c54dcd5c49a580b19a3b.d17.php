...
<p>Lots to go at here. &nbsp;<code class="inline blue">Display: none</code>keeps the element hidden on page load. &nbsp;The JavaScript will deal with fading it in and out in a moment. &nbsp;<code class="inline blue">z-index:33</code>keeps it &#8216;on top&#8217; of the page underneath. <code class="inline blue">height</code> and <code class="inline blue">width</code> are the same which would render a square except in modern browsers which support border-radius (IE 9+, Safari 5+, Chrome, FireFox 4+) which make it a circle, which I think is a pretty cool trick and avoides needing to create something in PhotoShop (I actually use GIMP). &nbsp;A solid green 1px border makes the circle show up. &nbsp;The positioning on the page is cntrolled by <code class="inline blue">position: fixed</code> which keeps the element positioned relative to the window, rather than the page. The <code class="inline blue">top: 50%</code> <code class="inline blue">right: 2%</code> keep it postioned relative to the height and width of the window. &nbsp;Have a play around with the window size and you will see it stay half way down, and the distance from the right gets bigger/smaller as the window size changes - quite nifty&#8230;</p>
